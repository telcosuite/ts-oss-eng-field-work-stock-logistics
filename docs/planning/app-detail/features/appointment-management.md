| Field | Value |
| --- | --- |
| Feature ID | F-appointment-management-01 |
...
- Field dispatcher can uses the appointment management capability to schedule appointments, assign work orders, balance technician or contractor capacity, for the persona-specific outcome `The appointment record keeps the field schedule executable with visible jeopardy…`, evidenced by the `## Persona & decision` audit trail in this file.
- Field technician can uses the appointment management capability to execute install, repair, survey, migration, pickup, maintenance, for the persona-specific outcome `The appointment record captures serials, photos, signatures, tests, site access,…`, evidenced by the `## Persona & decision` audit trail in this file.
- Warehouse/logistics manager can uses the appointment management capability to reserve, pick, pack, ship, transfer, consume, receive, quarantine, refurbish, for the persona-specific outcome `The appointment record keeps stock and shipment state aligned to work orders and…`, evidenced by the `## Persona & decision` audit trail in this file.
- Fulfillment operations lead can uses the appointment management capability to monitor field, appointment, stock, shipping, for the persona-specific outcome `The appointment record sees field jeopardy, manual fallout, partial completion, …`, evidenced by the `## Persona & decision` audit trail in this file.
- Inventory manager can uses the appointment management capability to accept, reject, or repair field-to-inventory handover evidence for installed, removed, swapped, quarantined, or returned resources. for the persona-specific outcome `The appointment record protects Resource Inventory as the final master while con…`, evidenced by the `## Persona & decision` audit trail in this file.
- Contractor/HSE coordinator can uses the appointment management capability to verify contractor assignment, site access, jsa/hse evidence, time/material claims, permits, for the persona-specific outcome `The appointment record proves partner labor and safety controls without giving c…`, evidenced by the `## Persona & decision` audit trail in this file.
- Customer/care handoff user can uses the appointment management capability to view appointment, technician eta, shipment, failed-appointment, completion, for the persona-specific outcome `The appointment record answers customer inquiries from app events without editin…`, evidenced by the `## Persona & decision` audit trail in this file.
...
- AC1 (AC-appointment-management-01): Given an authorized field dispatcher, field technician, warehouse/logistics manager, fulfillment operations lead, inventory manager, contractor/HSE coordinator, or customer/care handoff user creates or updates the appointment record, when the Appointment Management lifecycle advances, then Field Work, Stock, And Logistics validates service order readiness, work order dependency, technician or contractor availability, skill requirement, travel zone, customer/site access window, stock or shipment readiness, HSE/site restriction, tenant/market calendar, and blackout rules before accepting the state change.
- AC2 (AC-appointment-management-02): Given the appointment record references appointment, work order, stock, shipment, order, fulfillment, inventory, assurance, partner, ERP, customer, platform, or data records, when a persona opens the Appointment Management record, then the app shows source owner, source timestamp, freshness, dependency status, customer-impact flag, and whether the data is app-owned or read-only.
- AC3 (AC-appointment-management-03): Given Customer accepts a slot before required CPE or SIM/eSIM has shipped, when validation fails for Appointment Management, then the app keeps the appointment record in blocked, jeopardy, no-access, shortage, HSE stop-work, exception, rejected, rework, or fallout state with severity, owner, due date, affected customer/site/service/resource/order, retry/reschedule/rework option, and correlation ID.
- AC4 (AC-appointment-management-04): Given the appointment record changes due to mobile action, dispatcher action, warehouse action, carrier callback, contractor evidence, customer reschedule, inventory rejection, partner callback, or automation, when the transition is committed, then the app stores decision right, actor, role, reason, evidence links, old/new values, tenant/region boundary, and idempotency key.
- AC5 (AC-appointment-management-05): Given Order Management, Fulfillment Activation Control Tower, Inventory And Topology, Assurance/NOC, Customer/Care, Partner, Billing, ERP, Platform, or Data consumers subscribe to Appointment Management changes, when the appointment record reaches milestone, exception, no-access, shipment, stock, HSE, handover, or completion state, then the app emits a versioned event with changed fields, impact, SLA/OLA state, replay metadata, and correlation ID.
- AC6 (AC-appointment-management-06): Given a greenfield install, brownfield MACD, enterprise bulk rollout, partner/off-net dependency, automated dispatch, manual fallout, field visit, no-access, partial activation, rollback, migration, or decommissioning scenario references the appointment record, when the user requests closure, then the app validates downstream handoffs, open exceptions, evidence completeness, customer/order notification, inventory or stock acceptance, retention, and legal hold before closure.
- AC7 (AC-appointment-management-07): Given operations leaders review Appointment Management operations, when they open dashboards, then they see volume, queue aging, automation rate, SLA/OLA jeopardy, no-access, stock/shipment readiness, evidence rejection, HSE block, inventory handover status, partner/contractor aging, and completion quality linked to the appointment record.
